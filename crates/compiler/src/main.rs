...
use clap::Parser;
use core_lib::vis_parser::{self, ParserError};
...
fn run_check(args: &Args, config: &config::Config) {
    ...
    for entry in std::fs::read_dir(&config.input).expect("Failed to read input directory") {
        ...
        if path.is_file() && path.extension().and_then(|s| s.to_str()) == Some("vis") {
            ...
            match vis_parser::parse_vis(&contents) {
                Ok(_) => {
                    println!("  OK");
                }
                Err(errors) => {
