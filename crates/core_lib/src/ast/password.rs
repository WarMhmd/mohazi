...
pub struct PasswordRules {
    ...
    #[serde(alias = "minUpper")]
    pub min_upper: Option<RuleType<u128>>,
    #[serde(alias = "minLower")]
    pub min_lower: Option<RuleType<u128>>,
    #[serde(alias = "minNumbers")]
    pub min_numbers: Option<RuleType<u128>>,
    #[serde(alias = "minSymbols")]
    pub min_symbols: Option<RuleType<u128>>,
}

impl RuleTrait for PasswordRules {
    fn new() -> Self {
        Self {
            min_length: None,
            max_length: None,
    ...
    fn set_rule(&mut self, key: &str, value: Value, error: Option<String>) -> Result<(), String> {
        ...
        match key {
            ...
            "minUpper" | "min_upper" => {
                self.min_upper = Some(RuleType {
                    value: parse_val(value)?,
                    error: rule_err,
                })
            }
            "minLower" | "min_lower" => {
                self.min_lower = Some(RuleType {
                    value: parse_val(value)?,
                    error: rule_err,
                })
            }
            "minNumbers" | "min_numbers" => {
                self.min_numbers = Some(RuleType {
                    value: parse_val(value)?,
                    error: rule_err,
                })
            }
            "minSymbols" | "min_symbols" => {
                self.min_symbols = Some(RuleType {
                    value: parse_val(value)?,
                    error: rule_err,
                })
            }
    ...
    fn is_valid_key(key: &str) -> bool {
        match key {
            ...
            "uppercase" => true,
            "lowercase" => true,
            "minUpper" | "min_upper" => true,
            "minLower" | "min_lower" => true,
            "minSymbols" | "min_symbols" => true,
            "minNumbers" | "min_numbers" => true,
            _ => false,
        }
    }
}

impl Mergeable for PasswordRules {
    fn merge(&mut self, other: PasswordRules) -> Result<(), String> {
        ...
        if other.min_upper.is_some() {
            if self.min_upper.is_some() {
                return Err("Duplicate rule: min_upper".to_string());
            } else {
                self.min_upper = other.min_upper;
            }
        }
        if other.min_lower.is_some() {
            if self.min_lower.is_some() {
                return Err("Duplicate rule: min_lower".to_string());
            } else {
                self.min_lower = other.min_lower;
            }
        }
        if other.min_numbers.is_some() {
            if self.min_numbers.is_some() {
                return Err("Duplicate rule: min_numbers".to_string());
            } else {
                self.min_numbers = other.min_numbers;
            }
        }
        if other.min_symbols.is_some() {
            if self.min_symbols.is_some() {
                return Err("Duplicate rule: min_symbols".to_string());
            } else {
                self.min_symbols = other.min_symbols;
            }
        }
...
impl Mergeable for PasswordTransform {
...
impl TransformTrait for PasswordTransform {
    fn new() -> Self {
        PasswordTransform {
            trim: None,
            to_lowercase: None,
